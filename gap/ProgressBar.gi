...
PB_Global.Printer.PatternOptionsDefault := Immutable(rec(
	...
	# syncronize the width/height across all processes
	sync := [],
));
...
InstallGlobalFunction("PB_InitializeDimension", function(process, pattern)
	...
	# leaf node
		...
		options := ShallowCopy(PB_Global.Printer.PatternOptionsDefault);
		PB_SetOptions(options, pattern.options);
		dim := pattern.printer.dimension(process, pattern.printer_options);
		bounds := PB_GetBounds(process, pattern);
		for dir in ["w", "h"] do
			if dir in options.sync and process <> PB_Global.Process then
...
BindGlobal("PB_AllocateBlocks", function(process)
	...
	process.blocks := rec();
	pattern := PB_Global.Printer.Pattern;
	bounds := PB_GetBounds(process, pattern);
	bounds.x := 1;
...
BindGlobal("PB_PrintProcess", function(process, doGenerate)
	...
	if PB_PrintBlock(process, PB_Global.Printer.Pattern, doGenerate) = false then
		interlinked := PB_AllocateBlocks(process);
		PB_PrintBlock(process, PB_Global.Printer.Pattern, true);
	fi;
...
# 	root := PB_Global.Process;
# 	PB_Global.Terminal.screenWidth := SizeScreen()[1] - 1;
# 	PB_Global.Terminal.nr_digits := PB_NrDigits(PB_Global.ProcessMaxSteps(root));
# 	curProcess := fail;
# 	if IsBound(PB_Global.Terminal.curProcess) then
# 		curProcess := PB_Global.Terminal.curProcess;
# 	fi;
...
# 		t := PB_StrTime(PB_Global.ProcessTime(process));
# 		if t <> PB_Global.Terminal.totalTime then
# 			PB_Global.Terminal.totalTime := t;
...
# 		progress_ratio := Concatenation(String(curStep, PB_Global.Terminal.nr_digits), "/", String(nrSteps));
...
# 		# progress bar length
# 		bar_length := PB_Global.Terminal.screenWidth - Length(indent) - Length(options.bar_prefix) - Length(options.bar_suffix) - Length(progress_info);
# 		bar_length_full := Int(bar_length * r);
# 		bar_length_empty := bar_length - bar_length_full;
...
# 		PB_Print(String(curStep, PB_Global.Terminal.nr_digits));
# 		PB_MoveCursorRight(1 + PB_Global.Terminal.nr_digits);
# 		if options.printETA then
...
# 	PB_Global.Terminal.cursorVerticalPos := PB_Global.Terminal.cursorVerticalPos + nrLines - 1;
# 	PB_Global.Terminal.usedLines := Maximum(PB_Global.Terminal.usedLines, startLine + nrLines - 1);
# end);
